...
{% raw %}## maton google-sheets values update

```
maton google-sheets values update <spreadsheet-id> [flags]
```

Replace values in a range. Use --values for a simple comma-separated single row, or --json-values for richer payloads (single row or multi-row).
...
{% highlight bash %}{% raw %}
$ maton google-sheets values update ABC --range A1 --values 'Alice,100,true'
$ maton google-sheets values update ABC --range A1:B2 --json-values '[["x","y"],["a","b"]]'
{% endraw %}{% endhighlight %}
